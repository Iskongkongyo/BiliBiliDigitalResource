...
这是一个 B 站数字周边解析与下载工具，同时提供可直接打开的**本地手动版、本地自动版**，以及带安全代理与自动解析能力的 **Cloudflare Workers 云端版**。此外，本项目也提供独立的 **装扮资源提取工具**，用于提取 B 站装扮商城 `suit/detail` 页面里的主题皮肤、空间背景、评论卡片、表情包等素材。不想部署可以直接使用本地 HTML，想要更稳定、私有的体验也可以部署自己的 Worker。
