...
> 看到 B 站个性装扮里的炫酷的动态头像、精致的表情包和空间背景，却只能在 APP 里看着流口水？想保存下来当壁纸？
>
> **别慌，这个项目就是为了打破这些枷锁而生的！** 🚀

这是一个 B 站数字周边解析与下载工具，同时提供可直接打开的**本地手动版、本地自动版**，以及带安全代理与自动解析能力的 **Cloudflare Workers 云端版**。此外，本项目也提供独立的 **装扮资源提取工具**，用于提取 B 站装扮商城页面里不是9.9r抽卡的主题皮肤、空间背景、评论卡片、表情包等素材。
...
> [!TIP]
>
> 如果不想部署，可直接下载[手动版本.html](./本地部署/手动版本.html)、[自动版本.html](./本地部署/自动版本.html)或[装扮提取.html](./本地部署/装扮提取.html)到本地使用：
>
...
> - **装扮提取**：面向 B 站装扮商城付费限量的`应援装扮`的页面(分享链接路径以 `suit/detail` 结尾)，需要手动打开装扮接口并粘贴 JSON，可预览并打包下载主题皮肤、空间背景、评论卡片、表情包、头像挂件等素材。
> - **云端版本**：部署 [worker.js](./云端部署/worker.js) 后使用，支持自动解析、安全代理和自动失败后的手动兜底。
> - **通用镭射图展示**：下载好的B站带镭射图的所有数字周边均可使用。

## ✨ 核心亮点 

- **🪄 一键智能解析**：只需粘贴 B 站 APP 里的分享链接，自动拉取所有高清卡面、视频素材、动态头像和表情包。
- **🧥 装扮资源提取**：独立的 [装扮提取.html](./本地部署/装扮提取.html) 可提取装扮商城素材，包括封面、主题皮肤、空间背景、评论卡片、表情包、头像挂件和加载动画等。
...
- 图片、视频或镭射资源加载失败时，对应位置会显示“↻ 重新加载”，可直接再次请求。
- 打包下载时，原始卡面与镭射控制图都会保留，控制图位于 ZIP 内的 `镭射效果控制图` 文件夹。
- **镭射预览仅供参考，模拟效果可能与 B 站 APP 中的实际效果存在差异。**

### 装扮提取说明

- 打开 [装扮提取.html](./本地部署/装扮提取.html)，粘贴 B 站装扮商城 `suit/detail` 页面链接，例如 `https://www.bilibili.com/h5/mall/suit/detail?id=...`。
- 点击“打开装扮接口”，复制新页面返回的全部 JSON 文本，再粘贴回工具的接口数据输入框。
- 点击“解析并渲染资源”后，页面会按分类横向展示素材；图片或视频加载失败时，可点击“↻ 重新加载”再次请求。
- 支持点击“打包下载全部”生成 ZIP，也支持鼠标悬停在素材卡片上按 `S` 单独下载。
- 主题皮肤里的客户端离线资源包不会默认提取，避免出现无预览价值的 ZIP 文件和系统安全提示。
