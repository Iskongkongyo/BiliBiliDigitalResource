...
> [!CAUTION]
>
> 1、如果不想费时费力部署，可直接下载[index(手动版本).html](./index(手动版本).html)或[cors(自动版本).html](./cors(自动版本).html)到本地使用！这两者在操作上面有一定的区别！前者是全手动版本（需要在新弹窗获取数据并粘贴到“**获取数字周边信息**”和“**获取媒体数据**”输入框），后者是自动版本（只需粘贴链接到“**获取链接**”输入框并点击“一键智能解析”等待获取视频和图片即可）。
>
> 2、根目录下的项目文件没有限制并发，下载视频/图片较快（但下载单文件过大，文件过多时容易出现部分文件下载失败的情况）！
>
> 限制并发版下的项目文件限制并发数为2，即最多同时下载2个视频/图片，速度慢点但可以有效缓解大文件下载失败的情况！！！
...
1. 打开 B 站移动端 APP，进入 **我的 -> 个性装扮 -> 搜索你想找的装扮**，[点我一键直达！](bilibili://forward?-Btarget=https%3A%2F%2Fwww.bilibili.com%2Fh5%2Fmall%2Fhome%3Fnavhide%3D1)
