...
> 看到 B 站个性装扮里的炫酷的动态头像、精致的表情包和空间背景，却只能在 APP 里看着流口水？想保存下来当壁纸？**别慌，这个项目就是为了打破这些枷锁而生的！** 🚀
>

这是一个 B 站个性装扮解析与下载工具，同时提供可直接打开的**本地版**，以及带安全代理与自动解析能力的 **Cloudflare Workers 云端版**。
...
> [!TIP]
>
> 如果不想部署，可直接下载[本地版本.html](./本地部署/本地版本.html)到本地使用：
>
> - **本地版本**：支持自动和手动。切换到自动时粘贴链接后即可一键解析，使用公共代理获取数据；公共服务偶尔可能波动，失败时可改用手动版本。切换到手动时需要在新窗口获取 JSON，并粘贴到“获取数字周边信息”和“获取媒体数据”输入框。
...
- **🧥 装扮资源提取**：可提取装扮商城素材，包括封面、主题皮肤、空间背景、评论卡片、表情包、头像挂件和加载动画等。
...
<p align="center">
  <a href="https://deploy.workers.cloudflare.com/?url=https://github.com/Iskongkongyo/BiliBiliDigitalResource/tree/main/云端部署">
    <img src="https://deploy.workers.cloudflare.com/button" alt="Deploy to Cloudflare">
  </a>
</p>

部署时登录 Cloudflare，按页面提示选择账号完成部署即可。

### 第二步：配置环境变量

为了接口安全和私密性，**建议**在控制台配置以下变量（不用改代码）。这些内容包含密钥或密码，请选择 **Secret** 类型，不要保存成可见的纯文本变量：

进入 Worker 详情页 → **Settings** → **Variables and Secrets** → **Add**：

| **变量名**   | **类型** | **示例值**         | **作用说明** |
| ------------ | -------- | ------------------ | ------------ |
| `JWT_SECRET` | Secret   | 随机UUID | **选填**。用于签发代理访问令牌；不填写使用随机UUID。 |
| `BASIC_USER` | Secret   | `admin`            | **选填**。访问网页的账号；必须与 `BASIC_PASS` 同时配置。 |
| `BASIC_PASS` | Secret   | 一段高强度密码     | **选填**。访问网页的密码；必须与 `BASIC_USER` 同时配置。 |

> 修改变量后请点击 **Deploy** 使其生效。公开部署时建议设置 `JWT_SECRET`，因为Cloudflare 如果重启实例、扩容或切换到另一个 isolate，随机 UUID 就会变化，之前签发的 JWT Cookie 会立即失效，需要重新解析一次生成新 Token。
